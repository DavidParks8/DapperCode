...
impl ResolvedAgentManifest {
    ...
    /// Builds the isolated agent, appending bridge-supplied arguments to the manifest's own.
    ///
    /// The extra arguments are generated by the bridge rather than configured, so they are not
    /// part of the manifest and never change its digest.
...
mod tests {
    ...
    async fn isolated_agent_process_receives_only_baseline_and_manifest_environment() {
        ...
        let agent = candidate
            .acp_agent_with_args(std::slice::from_ref(&root), &host_environment, &[])
    ...
    fn manifest_requires_explicit_host_environment_values() {
        ...
        assert!(matches!(
            manifest.acp_agent_with_args(&[PathBuf::from("/bin")], &BTreeMap::new(), &[]),
    ...
    fn manifest_rejects_invalid_missing_and_changed_executable_digests() {
        ...
        assert!(matches!(
            changed.acp_agent_with_args(std::slice::from_ref(&root), &BTreeMap::new(), &[]),
    ...
    fn tree_is_recomputed_immediately_before_agent_construction() {
        ...
        candidate
            .acp_agent_with_args(std::slice::from_ref(&root), &BTreeMap::new(), &[])
            .expect("valid installation tree");
        std::fs::write(&dependency, b"module.exports = false;\n").expect("tamper dependency");
        assert!(matches!(
            candidate.acp_agent_with_args(std::slice::from_ref(&root), &BTreeMap::new(), &[]),
