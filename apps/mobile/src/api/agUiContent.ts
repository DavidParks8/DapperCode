...
export function partsMatchMessageContent(
  ...
  return (
    parts.map(renderAgUiCustomContent).filter(Boolean).join("\n") === content
  );
}
...
function renderStructuredContent(value: unknown, depth: number): string[] {
  ...
  if (type === "image") {
    ...
    const mimeType =
      nonEmptyString(entry.mimeType) ?? nonEmptyString(entry.mime_type);
    const source =
      url ?? (data && mimeType ? `data:${mimeType};base64,${data}` : null);
  ...
  if (type === "terminal") {
    const terminalId =
      nonEmptyString(entry.terminalId) ?? nonEmptyString(entry.terminal_id);
    return [
...
export function structuredTextRemainder(
  text: string,
  structured: string,
): string {
  ...
  const remainder =
    structured.slice(0, overlapStart) +
    structured.slice(overlapStart + trimmed.length);
