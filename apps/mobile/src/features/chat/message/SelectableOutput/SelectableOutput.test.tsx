...
function renderOutput(text: string): ReactTestRenderer {
...
function webView(tree: ReactTestRenderer) {
...
describe('SelectableOutput', () => {
  ...
  it('falls back to native selectable text when the WebView fails to load', () => {
    ...
    const fallback = tree.root.find(
      (node) => node.type === 'Text' && node.props['selectable'] === true,
    );
    expect(fallback.children.join('')).toBe('fallback line');
    act(() => tree.unmount());
  });
