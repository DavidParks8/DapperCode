...
import {
  bindWorkspaceResourcesRevalidation,
  WORKSPACE_REVALIDATE_DEBOUNCE_MS,
} from './workspaceLifecycle';

describe('workspace resource lifecycle revalidation', () => {
  beforeEach(() => jest.useFakeTimers());
  afterEach(() => jest.useRealTimers());

  it('revalidates on picker setup, reconnect, and foreground with debounce', () => {
    ...
    const cleanup = bindWorkspaceResourcesRevalidation(ws, revalidate, appState);
    jest.advanceTimersByTime(WORKSPACE_REVALIDATE_DEBOUNCE_MS);
    ...
    emitStatus(true);
    emitAppState('active');
    jest.advanceTimersByTime(WORKSPACE_REVALIDATE_DEBOUNCE_MS - 1);
    expect(revalidate).toHaveBeenCalledTimes(1);
