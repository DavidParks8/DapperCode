...
import {
  ...
  textMessage,
  upsertMessage,
} from '@bridge/agui/agUiMessageMutations';
import {
  appendToolArgs,
  applyActivityDelta,
  applyMessagesSnapshot,
  reduceCustomEvent,
} from '@bridge/agui/agUiToolAndCustomEventReducers';
import {
  applyJsonPatch,
  nonEmptyString,
  record,
...
} from '@bridge/agui/agUiStructuredAndTerminalReducers';
...
function reduceTextMessageStartEvent(
  ...
  if (envelope.event.type !== EventType.TEXT_MESSAGE_START) {
    return current;
  }
...
function reduceTextMessageContentEvent(
  ...
  if (envelope.event.type !== EventType.TEXT_MESSAGE_CONTENT) {
    return current;
  }
  const event = envelope.event;
  return appendText(
...
function reduceTextMessageChunkEvent(
  ...
  if (envelope.event.type !== EventType.TEXT_MESSAGE_CHUNK) {
    return current;
  }
  const event = envelope.event;
...
function reduceReasoningContentEvent(
  ...
  if (envelope.event.type !== EventType.REASONING_MESSAGE_CONTENT) {
    return current;
  }
  const event = envelope.event;
  return appendText(
...
function reduceReasoningChunkEvent(
  ...
  if (envelope.event.type !== EventType.REASONING_MESSAGE_CHUNK) {
    return current;
  }
  const event = envelope.event;
...
function reduceReasoningEncryptedValueEvent(
  ...
  if (envelope.event.type !== EventType.REASONING_ENCRYPTED_VALUE) {
    return current;
  }
  const event = envelope.event;
...
function reduceToolCallStartEvent(
  ...
  if (envelope.event.type !== EventType.TOOL_CALL_START) {
    return current;
  }
  const event = envelope.event;
  if (current.subagentToolCallIds[event.toolCallId]) {
    return current;
  }
...
function reduceToolCallArgsEvent(
  ...
  if (envelope.event.type !== EventType.TOOL_CALL_ARGS) {
    return current;
  }
  const event = envelope.event;
  if (current.subagentToolCallIds[event.toolCallId]) {
...
function reduceToolCallEndEvent(
  ...
  if (envelope.event.type !== EventType.TOOL_CALL_END) {
    return current;
  }
  const event = envelope.event;
  if (current.subagentToolCallIds[event.toolCallId]) {
...
function reduceToolCallChunkEvent(
  ...
  if (envelope.event.type !== EventType.TOOL_CALL_CHUNK) {
    return current;
  }
  const event = envelope.event;
  if (!event.toolCallId) {
...
function reduceToolCallResultEvent(
  ...
  if (envelope.event.type !== EventType.TOOL_CALL_RESULT) {
    return current;
  }
  const event = envelope.event;
  if (current.subagentToolCallIds[event.toolCallId]) {
...
function reduceActivitySnapshotEvent(
  ...
  if (envelope.event.type !== EventType.ACTIVITY_SNAPSHOT) {
    return current;
  }
  const event = envelope.event;
...
function reduceActivityDeltaEvent(
  ...
  if (envelope.event.type !== EventType.ACTIVITY_DELTA) {
    return current;
  }
  const event = envelope.event;
...
function reduceStateSnapshotEvent(
  ...
  if (envelope.event.type !== EventType.STATE_SNAPSHOT) {
    return current;
  }
...
function reduceStateDeltaEvent(
  ...
  if (envelope.event.type !== EventType.STATE_DELTA) {
    return current;
  }
...
function reduceStepStartedEvent(
  ...
  if (envelope.event.type !== EventType.STEP_STARTED) {
    return current;
  }
...
function reduceStepFinishedEvent(
  ...
  if (envelope.event.type !== EventType.STEP_FINISHED) {
    return current;
  }
...
function reduceRawEvent(
  ...
  if (envelope.event.type !== EventType.RAW) {
    return current;
  }
  const event = envelope.event;
