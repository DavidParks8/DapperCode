...
import { createAgUiThreadMessageState, type AgUiEventEnvelope } from '@bridge/agui/agUi';
import {
  findMessage,
  markRunTerminal,
  markTerminal,
...
import type { AgUiThreadMessageState } from '@bridge/agui/agUiMessagesState';
import { structuredTextRemainder } from '@bridge/agui/agUiContent';
  ...
  describe('reduceSubagentActivity', () => {
    ...
    it('uses unknown tool id fallback and spawning text for non-completed status', () => {
      ...
      if (message?.role === 'activity') {
        ...
        expect(message.content.subAgent?.receiverThreadIds).toEqual(['receiver-x']);
      }
    });
  });
