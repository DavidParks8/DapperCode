...
export function processTurnLifecycleEvents(
  ...
  if (event.method === 'turn/plan/updated') {
    ...
    if (threadId !== currentId) {
      ...
      if (planUpdate) {
        cacheThreadPlan(threadId, (previous) => buildNextPlanStateFromUpdate(previous, planUpdate));
      }
      return;
    }
  ...
  if (event.method === 'item/completed') {
    ...
    if (threadId !== currentId) {
      ...
      if (completedToolEvent) {
        cacheThreadActiveCommand(threadId, completedToolEvent.eventType, completedToolEvent.detail);
      }
    ...
    if (completedToolEvent) {
      cacheThreadActiveCommand(threadId, completedToolEvent.eventType, completedToolEvent.detail);
      pushActiveCommand(threadId, completedToolEvent.eventType, completedToolEvent.detail);
    }
      ...
      api
        ...
        .then((summary) => {
          ...
          const shouldPreserveRunning =
            !hasExplicitTerminalStatus && runWatchdogUntilRef.current > Date.now();
