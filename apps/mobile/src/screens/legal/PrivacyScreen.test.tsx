...
describe('PrivacyScreen behavior', () => {
  ...
  it('renders flat native sections without card chrome and with real bullet rows', async () => {
    const tree = await renderPrivacy(url);
    const root = tree.root as Queryable;
    expect(root.findAll((node) => /blur|gradient/i.test(typeName(node)))).toHaveLength(0);
    expect(hasText(root, '- Bridge token auth is enabled by default.')).toBe(false);
    expect(hasText(root, 'Bridge token auth is enabled by default.')).toBe(true);
