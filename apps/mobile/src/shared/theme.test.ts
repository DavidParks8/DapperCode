import { SYSTEM_FONT_FAMILIES } from '@shared/fonts';
import {
  AppThemeProvider,
  colors,
  createAppTheme,
...
describe('theme', () => {
  ...
  it('uses a restrained signature blue-violet accent per color scheme', () => {
    const dark = createAppTheme('dark');
    const light = createAppTheme('light');

    expect(dark.colors.accent).toBe('#C7BFFF');
    expect(dark.colors.accentPressed).toBe('#AFA0FF');
    expect(dark.colors.accentText).toBe('#000000');
  ...
  it('uses a composed indigo-lavender surface for sub-agent work', () => {
    ...
    expect(dark.colors.subAgentBg).toBe('#17152B');
    expect(dark.colors.subAgentAccent).toBe('#B8AEFF');
