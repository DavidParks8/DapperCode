...
describe('browserPreview', () => {
  ...
  it('extracts local preview URLs from mixed text', () => {
    ...
    ).toEqual(['http://localhost:3000/', 'http://127.0.0.1:5173/__vite_ping']);
  });

  it('ignores trailing markdown backticks around preview URLs', () => {
    expect(extractLocalPreviewUrls('Open `http://localhost:3000/` in browser')).toEqual([
      'http://localhost:3000/',
    ]);
  });
  ...
  it('dedupes and trims recent targets', () => {
    expect(
      dedupeRecentPreviewTargets(['3000', 'http://127.0.0.1:3000/', 'localhost:5173']),
    ).toEqual(['http://127.0.0.1:3000/', 'http://localhost:5173/']);
  });
  ...
  it('builds a desktop preview bootstrap URL when requested', () => {
    ...
    ).toBe('http://192.168.1.26:8788/app?sid=preview&st=token&vp=desktop&vw=1440&vh=900');
  });
  ...
  it('updates an existing preview URL with a different viewport preset', () => {
    expect(
      applyBrowserPreviewViewportPreset('http://192.168.1.26:8788/dashboard?foo=bar&vp=mobile', {
        preset: 'desktop',
        width: 1512,
        height: 982,
      }),
    ).toBe('http://192.168.1.26:8788/dashboard?foo=bar&vp=desktop&vw=1512&vh=982');
  });
  ...
  it('maps proxied backend preview navigation URLs back to their loopback origin', () => {
    expect(
      mapBrowserPreviewNavigationUrlToTargetUrl(
        'http://100.108.165.85:8788/__dappercode_proxy__/aHR0cDovLzEyNy4wLjAuMTozMDAz/api/waitlist?source=landing',
        'http://100.108.165.85:8788',
        'http://127.0.0.1:3000/',
      ),
  ...
  it('handles external, root proxy, invalid proxy, and malformed navigation URLs', () => {
    ...
    expect(
      mapBrowserPreviewNavigationUrlToTargetUrl(
        'http://preview.test/__dappercode_proxy__/aHR0cDovLzEyNy4wLjAuMTozMDAw?sid=x',
        'http://preview.test',
    ...
    expect(
      mapBrowserPreviewNavigationUrlToTargetUrl(
        'http://preview.test/__dappercode_proxy__//fallback?frame=1&keep=yes#top',
        'http://preview.test',
        'http://127.0.0.1:4000/base',
      ),
    ...
    expect(
      mapBrowserPreviewNavigationUrlToTargetUrl(
        'http://preview.test/__dappercode_proxy__/aGVsbG8/bad',
        'http://preview.test',
        'http://127.0.0.1:4000/',
      ),
    ).toBeNull();
